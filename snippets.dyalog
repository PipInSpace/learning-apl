...
⍝ ⍺: data as bytes
⍝ ⍵: key as bytes
aes128←{∊{from_bits¨addRoundKey shiftRows to_bits¨subBytes from_bits¨{addRoundKey mixColumns shiftRows to_bits¨subBytes from_bits¨⍵}⍣9 ⊢ addRoundKey ⍵}¨block16 ⍺ ⊣ keys←expandKeys ⍵}
...
⍝ converts a vector ⍵ into a vector of 4x4 matrices
block16←{{4 4⍴⍵}¨↓((⊃⌈(⍴⍵)÷16) 16)⍴⍵}
